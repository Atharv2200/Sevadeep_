...
- Location: the browser sends latitude, longitude, accuracy; the server computes `distanceMeters` (Haversine) and never trusts a client distance. Reject only outside the radius; rejected attempts create no record. `accuracy` is not a rejection criterion (Phase 5.5) — it is stored purely as evidence and feeds the informational `LOW_ACCURACY` flag. Store submitted coordinates, accuracy and distance.
...
- Check-out (`POST /api/activities/:id/attendance/check-out`): authenticated volunteer + valid location (same radius rule as check-in; accuracy is not a rejection criterion here either), no QR. Allowed for OPEN or CLOSED (not CANCELLED) activities until the window's `closesAt`; a conditional update on `checkedOutAt: null` makes a second check-out `ALREADY_CHECKED_OUT`. Duration is derived, never client-submitted.
- Check-in order: ACTIVE volunteer -> activity exists (DRAFT is 404) -> status OPEN (`ACTIVITY_CLOSED`/`ACTIVITY_CANCELLED`) -> window via `utils/attendanceWindow` (`ATTENDANCE_WINDOW_CLOSED`) -> QR token -> distance (`OUT_OF_RADIUS`, 422) -> insert; the unique index answers `ALREADY_CHECKED_IN`. Rejected attempts store nothing. Attendance is rate-limited per signed-in user (not per IP: a venue shares one address).
...
- Shared layout width: `.container-custom` (`index.css`) is `max-w-[90rem] mx-auto`, used by the public site, `AppShell` (admin/volunteer header and main) and every page — one width for the whole app, not a per-page choice.
- Public-page vertical rhythm: `.section-padding` (`index.css`) is `py-14` and `Hero` is `min-h-[85vh]` (not `min-h-screen`), deliberately — a full `100vh` hero with no visible edge of the next section, stacked with generous section padding, is what made the site look "zoomed in" at real 100% browser zoom (confirmed by simulating a zoomed-out view: shrinking these same values let the next section peek into frame, exactly reproducing the "looks right at 80–90% zoom" complaint). There is no root-font-size hack, CSS `zoom`/`transform: scale`, or viewport-meta issue anywhere in the app — don't reintroduce a full-viewport-height section or restore the old padding values without re-checking this.
- Public home page (`pages/public/Home.jsx`): `Hero` -> `About` -> `Activities` (the six real activity categories, matching `ACTIVITY_CATEGORIES`) -> `GetInvolved`. There is no Stats or Gallery section: v1 has no real, backend-derived public statistics (`GET /api/stats/admin` is admin-only, by design) and no real Sevadeep photography, and fabricated numbers/stock photos are not acceptable placeholders — the sections were removed rather than invented. Add them back only once real data/images exist.
- No public contact channel exists (no real address/phone/email/socials on hand, no backend to receive messages) — `sections/GetInvolved.jsx` replaces the old fake contact form and footer contact block with a CTA into the one thing the platform actually does: `/register` and `/login`. Do not reintroduce a decorative contact form or invented contact details; if real contact info or a message-handling backend is added later, wire it here.
...
5. Secure attendance, QR and location (Attendance model, `qrTokenService`, `attendanceService`, `utils/geo`, check-in/out, live attendance, `/attend/:activityId`) — done. Phone testing needs an HTTPS tunnel; the Vite dev/preview server's hosts are widened only through `VITE_ALLOWED_HOSTS` (never `*`). `npm run dev:phone` (frontend) runs the same dev server as `npm run dev` bound to the LAN interface (`vite --host`) for the tunnel to reach — see README "Testing attendance on a phone".
